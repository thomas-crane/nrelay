...
export class HttpClient {
     ...
     */
    static get(path: string, options: IRequestOptions = { query: {}, proxy: null }): Promise<string> {
    ...
    private static getWithProxy(endpoint: url.Url, proxy: IProxy, query: string): Promise<any> {
        return new Promise((resolve: (data: string) => void, reject: (err: Error) => void) => {
            ...
            Log(provider, 'Establishing proxy for GET request.', LogLevel.Info);
            SocksClient.createConnection({
                ...
                Log(provider, 'Established proxy!', LogLevel.Success);
                let data = '';
