...
import { Account, Server, GameId } from '../models'
...
export class Runtime extends EventEmitter {
  ...
  addClient(account: Account): Promise<Client> {
    ...
    //make sure if gameId is set... it is a valid gameId
    if (account.gameId && !GameId[account.gameId]) {
      account.gameId = GameId.Nexus
      Logger.log(account.alias, `Preferred gameId not found. Using GameId.Nexus instead.`, LogLevel.Warning)
    }
    ...
    return Promise.all([this.accountService.getServerList(), this.accountService.getCharacterInfo(account.guid, account.password, account.proxy)]).then(([servers, charInfo]) => {
  ...
  removeClient(guid: string): void {
    ...
    if (this.clients.has(guid)) {
      ...
      Logger.log('Runtime', `The client ${censorGuid(guid)} is not part of this runtime.`, LogLevel.Warning)
    }
  }
