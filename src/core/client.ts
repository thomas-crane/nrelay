...
export class Client {
  ...
  constructor(runtime: Runtime, server: Server, accInfo: Account) {
    ...
    this.key = []
    this.keyTime = -1
    this.internalGameId = accInfo.gameId
  ...
  shoot(angle: number): boolean {
    ...
    for (let i = 0; i < numProjectiles; i++) {
      ...
      const newProj = new Projectile(item.type, containerProps, 0, this.objectId, shootPacket.bulletId, angle, time, {
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (this.projectiles[i].damagePlayers) {
        ...
        if (insideSquare(this.projectiles[i].currentPosition, this.worldPos, 0.5)) {
          ...
          const nexused = this.applyDamage(this.projectiles[i].damage, this.projectiles[i].projectileProperties.armorPiercing)
